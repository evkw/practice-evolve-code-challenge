...
import {
  selectTimesheetDraft,
  selectSelectedTimesheetEntries
} from '../selectors/timesheet.selector';

@Injectable()
export class TimesheetEffects {
  @Effect()
  loadTimesheetsStart$ = this.actions$.pipe(
    ofType<actions.LoadTimesheetsStart>(
      actions.TimesheetActionTypes.LoadTimesheetsStart
    ),
    mergeMap(() =>
  ...
  saveDraftTimesheetStart$ = this.actions$.pipe(
    ofType<actions.SaveTimesheetDraftStart>(
      actions.TimesheetActionTypes.SaveTimesheetDraftStart
    ),
    withLatestFrom(this._store.select(selectTimesheetDraft)),
    mergeMap(data =>
      this._timesheetSvc.saveDraft(data[1]).pipe(
        mergeMap(() => [
  ...
  @Effect()
  updateTimesheetStart = this.actions$.pipe(
    ofType<actions.UpdateTimesheetStart>(
      actions.TimesheetActionTypes.UpdateTimesheetStart
    ),
    mergeMap(action =>
      this._timesheetSvc.updateTimesheetEntry(action.payload).pipe(
        mergeMap(() => [
          new actions.UpdateTimesheetSuccess(),
          new actions.LoadTimesheetsStart()
        ]),
        catchError(error => of(new actions.SaveTimesheetDraftFailure(error)))
      )
  ...
  submitTimesheetEntriesStart$ = this.actions$.pipe(
    ofType<actions.SubmitTimesheetEntriesStart>(
      actions.TimesheetActionTypes.SubmitTimesheetEntriesStart
    ),
    withLatestFrom(this._store.select(selectSelectedTimesheetEntries)),
    mergeMap(data =>
      this._timesheetSvc.submitEntries(data[1]).pipe(
        mergeMap(() => [
          new actions.SubmitTimesheetEntriesSuccess(),
          new actions.LoadTimesheetsStart()
        ]),
        catchError(error =>
          of(new actions.SubmitTimesheetEntriesFailure(error))
        )
      )
    )
  );
